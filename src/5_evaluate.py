...
class LicensePlateSystem:
    ...
    def process(self, image_paths):
        results = []
        print(">> Przetwarzanie (Cascade Mode: Fast -> Heavy)...")

        det_results = self.detector(
            image_paths, imgsz=640, conf=0.25, verbose=False, stream=False
        )

        # Prekompilacja regexa walidacyjnego (Polska tablica: 2-3 litery + cyfry/litery)
                ...
                if crop.size > 0:
                    try:
                        ...
                        if h_cut > 0 and w_cut > 0:
                            ...
                            # WALIDACJA: Jeśli wygląda idealnie, kończymy (Oszczędzamy czas!)
                            if bool(pattern.match(text1)):
                                ...
                                # Wybieramy ten, który bardziej pasuje do wzorca lub jest dłuższy
